...
globals [greens greens_1000 avg_greens target no_neighbors green_neighbors avg_deg prob_of_adopt init_greens curr_greens new_greens]
...
to step2
  ask turtle target [
    ...
    [
      set prob_of_adopt p + q * green_neighbors
      if prob_of_adopt < 0 [set prob_of_adopt 0]
...
The Bass model (or Bass diffusion model) describes how new products become adopted by the population. Its original formulation proposed by Bass [1] conists of a simple differential equation. It takes into account two factors that support spreading of product, namely advertising (represented by coefficient of innovation) and word-of-mouth (as coefficient of imitation).

Here we present agent-based variant of the model [2] as well as its extension incorporating negative word-of-mouth [5].  
...
2) If target has not adopted the innovation yet, it adopts with probability p+q* n<sub>adopt, i </sub> / n*<sub> i </sub>, where n<sub>adopt, i </sub> denotes number of target's neighbors, who already adopted and n<sub>i</sub> is total number of target's neighbors
3) Go to 1
...
Choose parameters of the model:
  ...
  * p - coefficient of innovation (advertising)
  * q - coefficient of imitation (word-of-mouth)

INITIAL CONDITIONS
The last thing to choose is the initial fraction of adopters, given by the parameter:

  * density_of_adopters - the fraction of adopters at the beginning of simulations; they are randomly distributed over the whole system.
...
1) "target" - an agent, which opinion will be updated is selected. 
2) "neighbors" - thick black lines indicate the target's neighbors. If target has not adopted the innovation yet, the probability of adoption is calculated and displayed. 
...
Typical values of p and q when time t is measured in years [6]:

  * The average value of p has been found to be 0.03, with a typical range between 0.01 and 0.03.
  * The average value of q has been found to be 0.38, with a typical range between 0.3 and 0.5.


Note that in the original formulation, the model will always lead to the state when the whole population adopts the innovation for non-zero value of p and non-negative q. You can observe how the interplay between these two parameters impact rate of adoption. 


However, if we incorporate negative word-of-mouth, we can observe steady state when only some part of the populations adopts. 


Try the following settings
  * N = 101
  * k = 100
  * density_of_adopters = 5%

Then set

  * p = 0.015 and q = 0.3 - the whole population adopts the innovation
  * p = 0.015 and q = -0.02 - only some fraction (around 0.75) of the population adopts 
